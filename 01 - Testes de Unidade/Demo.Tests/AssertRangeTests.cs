...
public class AssertRangeTests
{
    ...
    {
        // Arrange & Act
        var funcionario = new Funcionario("Samuel", salario);
        ...
        if (funcionario.NivelProfissional == NivelProfissional.Pleno)
            Assert.InRange(salario, 2000,7999);

        if (funcionario.NivelProfissional == NivelProfissional.Senior)
            Assert.InRange(salario, 8000, double.MaxValue);

        Assert.NotInRange(salario, 0, 499);
    }
